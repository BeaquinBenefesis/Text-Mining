...
from python.src.syngrep import run_syngrep
from python.src.models import HitType, NormalizedHit
from python.src.hitsUtils import HitsProcessor
...
class Test:
    ...
    def _reset(self):
        ...
        ).get_hits(sort=True, print_summary=False)
    ...
    @staticmethod
    def group_by_sentence(hits: Iterator[NormalizedHit]) -> dict[str, list[NormalizedHit]]:
    ...
    def _concepts_match(self, pred_hit: NormalizedHit, gold_hit: NormalizedHit) -> bool:
        gold_id = gold_hit.synonym_id
        if gold_id not in self.all_ids:
            return False
        return gold_id in self.id_map.get(pred_hit.synonym_id, set())

    def _gold_type(self, gold_hit: NormalizedHit) -> str:
        return getattr(gold_hit, 'synonym', 'UNKNOWN')
    ...
    def _score(self, tp_pairs, fp_preds, fn_golds, track):
    ...
    def run(self, mode: str, track: str):
        self._reset()
        ...
        self.print_summary(mode, track)

    def print_summary(self, mode, track):
...
test.run('exact', 'recognition')
test.run('overlap', 'recognition')
test.run('exact', 'normalization')
test.run('overlap', 'normalization')
